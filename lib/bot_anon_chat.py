...
def register(client):
    """Daftarkan handler @botchatanonymouss_bot, kembalikan coroutine start()."""

    state = {"promo_sent": False, "busy": False}

    async def next_partner():
        logger.info("[Bot5] /next → cari pasangan baru")
        state["promo_sent"] = False
        await client.send_message(TARGET, '/next')

    async def search():
        logger.info("[Bot5] /search → mulai mencari")
        state["promo_sent"] = False
    ...
    async def handle(event):
        ...
        if MATCH_TEXT in msg or GREET_TEXT in msg:
            ...
            state["busy"] = True
            try:
                logger.info("[Bot5] Match! Jeda 5 detik...")
                await asyncio.sleep(5)
                if await safe_send_message(client, TARGET, LINK, label='Bot5'):
                    state["promo_sent"] = True
                    stats.increment('botchatanonymouss_bot')
                    logger.info("[Bot5] Promo terkirim!")
        ...
        # Partner pergi sebelum promo sempat dikirim → /next lagi
            ...
            logger.info("[Bot5] Lawan pergi/berhenti. /next lagi...")
            await asyncio.sleep(3)
            await next_partner()

        elif SEARCH_TEXT in msg:
            logger.info("[Bot5] Sedang mencari obrolan...")
